...
The sound devices for production use have unique features which usual sound devices don't have. Some of the features require softwares to handle them. In this section, I describe such features to consider software implementation.
...
In the DSP side, outgoing packets are the sink of audio signals or the source of audio signals. The DSP is between IEC 61883-1/6 layer and DAC/ADC for audio ports.

The way to control the DSP behaviour depends on each chipset or model\cite{bebob-1}.
...
Moreover, in previous subsection, the sequence of value in SYT field can generate a pulse and the pulse can be selected as source of clock. This option means that a device can delegate 'timing master' to the transmitter.


\subsection{Different formation of data channel in a packet}

In IEC 61883-6\cite{iec61883-6-1, iec61883-6-2}, a packet can include several types of data, such like PCM samples, MIDI messages and IEC 60958 Conformant data. Each data is transferred by each data channel in packets. Each model has its own formation of data channels, depending on sampling frequency or a certain modes.
...
The way to retrieve information about the formation depends on each chipset.


\subsection{Chipset specific operations and quirks}

Although there are some public specifications, each device has each quirks due to chipset or vendor's customization. Currently quirks of four types of chipset are clear.
...
Fireworks use its own mechanism for transaction. Drivers transmit commands to a specific address on device, and receive responses to a specific address on controller. Fireworks has unique command sets. Although this transaction is transferred to specific addresses, there's another way to transfer commands and responses as AV/C vendor specific commands.

Fireworks manages streams by CMP. Fireworks transfers AMDTP packets in blocking mode but Fireworks streams are not fully compliant to IEC 61883-1. In IEC 61883-1, the value of data block counter means the number of packets which has already transferred. But in Fireworks, it means the number of data blocks which has transferred including current packet.
...
BeBoB based devices can be controlled by CMP and vendor specific AV/C commands\cite{bebob-1,bebob-2}. BeBoB transfers AMDTP packets in blocking mode. BeBoB can be customized largely so there're some model specific operations out of its specification.


\subsubsection{OXFW970/971}
OXFW970/971 was produced by Oxford Semiconductor. OXFW970/971 based devices can be controlled by CMP and generic AV/C commands. OXFW970/971 transfers AMDTP packet in non-blocking mode.


\subsubsection{Dice}
Dice is Digital Interface Communications Engine which TC Applied Technologies produces.

Dice doesn't use CMP or any AV/C commands, although Dice can be controlled by read/write transactions to specific register. Dice has unique mechanism to notify status change. This notification is delivered to a specific address on controller.

Dice transfers AMDTP packet in blocking mode. But Dice is not fully compliant to IEC 61883-6. At higher sampling rate, Dice transfers AMDTP packets with double count of data blocks at a half of rate than IEC 61883-6. For example, in IEC 61883-6, at 192.0kHz in blocking mode, an AMDTP packet includes 32 data blocks. But Dice transfers AMDTP packets with 64 data blocks at 96.0kHz.


\section{Existing drivers}

\subsection{FFADO}
FFADO is a project to develop user-land drivers for firewire sound devices and FreeBoB is a ancestral project of FFADO. Daniel Wagner started this project to develop driver for BridgeCo Enhanced Breakout Box (BeBoB) based devices in 2004. Daniel was in BridgeCo AG and had a contract to disclose relevant resources to the other developers. Later, Peter Palmer joined, The code base of FFADO grew up by them. In 2006, Jonathan Woithe joined in this project. He worked for drivers for MOTU devices. In 2007, to support more devices, FFADO project began.
...
Currently, there are no FFADO applications except for 'ffado-dbus-server' and JACK server daemon (jackd). The jackd uses libffado to packetize and transfer packets between devices. The jackd uses UNIX domain socket and System V Shared Memory to gather/distribute data to/from JACK application processes. 
...
As of 2013, ALSA firewire stack generally supports PCM playback only, while FFADO supports more functionality. If there're some ways to bridge ALSA and FFADO, It doesn't matter that ALSA firewire stack is so weak. In next section, I investigate ALSA implementation in user-land to seek a solution for this issue.
...
FCP functionality gives a function, fcp\_avc\_transaction() for FCP and AV/C commands. The drivers can transmit FCP request and wait for FCP response for 125 micro seconds. All of FCP requests should be retried when bus reset occurs, so fcp\_bus\_reset() is used for this purpose.
...
This module gives a structure for AMDTP stream and some operations to handle an instance of the structure. This functionality uses the continuous packets buffer functionality internally. Drivers initialize an instance of the structure by amdtp\_stream\_init(). After initializing, amstp\_stream\_set\_parameters() is used to set streaming parameters such as sampling rate, the number of Multi Bit Linear Audio (MBLA) data channel or MIDI Conformant data channel for the stream. After setting parameters, amdtp\_stream\_get\_max\_payload() returns the maximum number of bytes of payload for a packet with current parameters.
...
After queueing packets and isochronous context starts, firewire-ohci callbacks according to an 'interrupt' flag of packet structure.

\begin{figure}[H]
	\centering
	\includegraphics{./img/packetization-former.svg.eps}
	\caption{{Packetizing process}}
	\label{{fig:packetization-former}}
\end{figure}
...
\begin{itemize}
\item Retrieving device capability by transaction
\item Notifying to start/stop streaming
\item Locking/Unlocking streaming
\item Giving Firewire node information
\end{itemize}
...
This work renames prefix of 'amdtp\_out\_stream' to 'amdtp\_stream' for all of functions and members of AMDTP stream structure. This means the same structure is used for both directions.
...
This work adds an argument for amdtp\_stream\_init(), to indicate the direction of AMDTP stream. After initialized, the instance of stream can be operated by the same way for both direction.

This work newly adds amdtp\_stream\_wait\_callback() to wait for a first packet until given timeout. This is for detection for the case that drivers set wrong parameters for packets, i.e. tag. In this case, OHCI 1394 driver doesn't execute callback function of isochronous context. Then no packets are handled, and PCM component don't waken a process of PCM substream because 'avail' space has never been updated (section \ref{sec:alsa-pcm} to page \pageref{sec:alsa-pcm}). This function is expected to be used just after staring a stream. Drivers should handle error state when receive false.
...
As a default, any AMDTP streams are not associated. For duplex streams with timestamp synchronization (section \ref{sec:duplex-streams} to page \pageref{sec:duplex-streams}), this work adds amdtp\_stream\_set\_sync(). This function associates two streams as a timing master and a timing slave with given synchronization mode, and this function should be called before starting streams. Currently CIP\_SYNC\_TO\_DEVICE is effective for this parameter. The case to use this option is that a timing master is incoming stream and a timing slave is outgoing stream. In this case, the value of SYT field in incoming packets are passed to outgoing packets. The packets in both direction are handled in interrupt context for an incoming isochronous context. In the other cases, there's no requirement to pass the value of SYT field between two streams.

This work adds a functionality to detect discontinuity of the sequence of incoming AMDTP packets. In a case of the detection, the stream is stopped and corresponding PCM substream is aborted. In this case, the stream should be stopped and started again.
...
For PCM component, this work newly adds amdtp\_stream\_add\_hw\_constraints() to add PCM constraint according to parameters of AMDTP stream. This function should be called after all of needed flags are set to an instance of AMDTP stream structure. This work also enables drivers to call amdtp\_stream\_set\_pcm\_format() during streaming. This is for the case that PCM substream is going to join in AMDTP stream, which MIDI substream already starts.
...
This work enables firewire-lib to handle deferred transaction. According to AV/C General specification\cite{avc-general-4-2}, receivers of AV/C command should respond to transmitter within 100 micro seconds, while the receivers send an intermediate response within 100 micro seconds, and send a final response later. This is deferred transaction. The interval between the intermediate and final response is undefined. But to promise to return to caller, firewire-lib uses 125 micro seconds interval.
...
In MIDI specification, the phy layer can transfer 3,125bit per sec. But in IEC 61883-6\cite{iec61883-6-1, iec61883-6-2} and MMA/AMEI RP-027\cite{amei-rp27}, the stream can transfer MIDI messages by one eighth of sampling rate, for example, 6,000 bytes at 48.0kHz. So devices must buffer the gap between these data rate.
...
Currently, the minimum number of frames per period is not less than 5 micro seconds. The drivers PCM component has a flag, SNDRV\_PCM\_INFO\_BATCH and the minimum number of periods in buffer is 2. As a result, the minimum latency is over 10 micro seconds. For more severe latency, further work is needed.
