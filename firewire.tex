...
In IEC 61883-6\cite{iec61883-6-1, iec61883-6-2}, a packet can include several types of data in each data block, such like PCM samples, MIDI messages and IEC 60958 Conformant data. Each data is transferred by each data channel in packets. Each model has its own formation of data channels, depending on sampling frequency or a certain modes.
...
Fireworks manages streams by CMP and transfers AMDTP packets in blocking mode. But Fireworks streams are not fully compliant to IEC 61883-1. In IEC 61883-1, the value of data block counter means the number of packets which has already transferred. But in Fireworks, it means the number of data blocks which has transferred including current packet.
...
BeBoB based devices can be controlled by CMP and vendor specific AV/C commands\cite{bebob-1,bebob-2}. But some BeBoB based devices needs both connections to transmit AMDTP outgoing stream. BeBoB transfers AMDTP packets in blocking mode. BeBoB can be customized largely so there're some model specific operations out of its specification.
...
Currently, there are no FFADO applications except for 'ffado-dbus-server' and JACK \footnote{http://jackaudio.org/} server daemon (jackd). The jackd uses libffado to packetize and transfer packets between devices. The jackd uses UNIX domain socket and System V Shared Memory to gather/distribute data to/from JACK application processes. 
...
This work add new functionalities to ALSA firewire stack. So new drivers can be developed and existed drivers can be updated for both of PCM/MIDI playback/capture.

PCM functionality of each driver has SND\_PCM\_INFO\_BATCH flag because the driver uses a part of packet queue for each PCM period.

PCM functionality also has SND\_PCM\_INFO\_BLOCK\_TRANSFER flag because PCM frames are transferred at each interrupt by the number of data blocks in some packets \footnote{QUEUE\_LENGTH in /sound/firewire/amdtp.c}.

PCM samples are copied to each data blocks in its order and the posision of PCM samples in a data block is linear, so SNDRV\_PCM\_INFO\_INTERLEAVED should be used.

In most case, incoming/outgoing AMDTP streams should be the same sampling rate, so SNDRV\_PCM\_INFO\_JOINT\_DUPLEX should be used. Each drivers should restrict sampling rate for both playback/capture as the same.
