...
\begin{itemize}
	...
	\item Signal with recovered clock as described in previous subsection (4.0/5.512/6.0kHz)
\end{itemize}
...
In IEC 61883-6\cite{iec61883-6-1, iec61883-6-2}, an event is represented as a data block. The data block can include several types of data in each data channel. For example, Multi Bit Linear Audio (MBLA) data channel for PCM samples, MIDI Conformant data channel for MIDI messages and IEC 60958 Conformant data channel for PCM samples. Each model has its own formation and the number of data channels, depending on sampling frequency or a certain modes, even if these models use the same chipset.
...
Fireworks uses its own mechanism for transaction. Drivers transmit commands to a specific address on device, and the device transmit responses to a specific address on controller. Fireworks, therefore, has unique command sets. Although this transaction is transferred to specific addresses, there's another way to transfer commands and responses as AV/C vendor specific commands.

Fireworks manages streams by CMP and transfers AMDTP packets in blocking mode. But Fireworks streams are not fully compliant to IEC 61883-1. In IEC 61883-1, the value of data block counter means the number of packets which has already transferred. But in Fireworks, it means the number of data blocks which has transferred including current packet. Furthermore, its empty packets are transferred at tag b00 in isochronous packet. Additionally, Fireworks has more quirks depending on firmware version for iceLynx Micro.
